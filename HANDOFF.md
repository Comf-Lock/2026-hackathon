...
created: 2026-06-25
last_updated: 2026-06-26-agent-4-header-stable
schema_version: "0.4"
status: slice1-deployed · master-orchestration · agent-4-header-stable
---
...
agent/agent-4: Header positionsstabil inkl. Profilseite **fertig + gepusht** [72d648e, auf rebased origin/master]. Ursache: `wideHeader` in App.vue listete `profile` nicht → `/profile` fiel auf die schmale 680px-Header-Bar zurück, Logo+Tabs sprangen beim Navigieren. Fix: `'profile'` zur wideHeader-Liste hinzugefügt → alle logged-in Views (dashboard/landing/calendar/map/profile) nutzen die 1240px/22px-Shell, Header pixelstabil. ProfileView spiegelt bereits die 854px-Content-Spalte (= Dashboard-Hauptspalte) → Breiten-Anforderung schon erfüllt, keine ProfileView-Änderung nötig. NUR `App.vue` geändert (Kommentar + computed). vite build grün (63 modules). Brief → `_scrape/processed/`.
> **Offen — PWA Teil B (Responsive):** Inbox-Brief `_scrape/inbox/task-pwa-phase1-mobile.md` Teil B (alle Views mobil-korrekt ~360–430px) bewusst noch nicht gemacht; Brief bleibt in `_scrape/inbox/`. PWA-Shell (Phase 1a) ist fertig [f26db4e]: vite-plugin-pwa, manifest.webmanifest (standalone, theme #b8324f / bg #f7f6f3, Icons 192/512/maskable), Workbox-SW (App-Shell-Cache, /api-Denylist), index.html Metas + apple-touch-icon → installierbar. On-Device-Test braucht HTTPS (Master).
> **Offen — PWA Teil B (Responsive):** Der Inbox-Brief `_scrape/inbox/task-pwa-phase1-mobile.md` enthält zusätzlich Teil B (alle Views mobil-korrekt: Nav/Liste/SearchMask/EventCard/Dashboard/Kalender+Detailspalte/Karte/Profil auf ~360–430px). Bewusst **noch nicht** gemacht (Lars: erst Shell). Brief bleibt deshalb in `_scrape/inbox/` (NICHT nach processed/).
> Vorherige Arbeit (bleibt PR-ready): Logged-out-Layout+Login/Logout+Tab „Suche" [98f0658]; Kalender-Detail-Spalte [1367c43] inkl. `calendar/CalendarEventDetail.vue`; Refactor P1.5 Auth-Redirect [f80c74d]; View-Tabs /calendar [2ad1d5d], /map [1bbf33b] inkl. MapEventList [fe1c441]; Architektur-Audit `plans/refactor-architecture-audit.md` [e0e2478].
