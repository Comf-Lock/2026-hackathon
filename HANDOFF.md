...
created: 2026-06-25
last_updated: 2026-06-26-agent-4-map-mobile-done
schema_version: "0.4"
status: master-orchestration · agent-4-map-mobile-done
---
...
agent/agent-4 (sole frontend agent). **Karten-Tab mobil — FERTIG, committet, vite build grün.** MapView: bei <880px stapelt Karte oben (volle Breite, feste Höhe `leaflet-host` 56vh/min320, <640px 52vh/min300) + Liste darunter; Liste flowt jetzt mit der Seite (`.side max-height:none`, kein nested-scroll-trap). **Leaflet-Falle gefixt:** `refreshMapSize()` ruft `map.invalidateSize()` nach `nextTick` beim Mount, nach Daten-Load UND auf `window 'resize'` (Listener in onBeforeUnmount entfernt) → keine grauen Tiles beim Stapeln/Breakpoint-Wechsel. <640px mapwrap-Padding 14px, Titel 18px. MapEventList (hatte 0 Media-Queries): @640px Header-Controls full-width (segmented `flex:1`, seg touch-padding 8px, radius-check full-width), `.lcount` full-width, Items größer (padding 11px, Titel 14px), interner Scroll aus (`.items overflow-y:visible`, `.sidelist height:auto`). Pin-Popups/flyTo unverändert. Nur MapView.vue + MapEventList.vue, Desktop unangetastet. Brief → `_scrape/processed/`. **Kontext:** frühere agent-4-Arbeit in master gemergt (zuletzt PR #61); Branch nach Rebase == master, HANDOFF aus ORIG_HEAD restored (Rebase bricht `merge=ours`). **Nächster Schritt:** push agent/agent-4 → Master merged via PR. Danach kein offener Auftrag — auf neuen Master-Brief/Lars-Input warten. **HINWEIS:** On-Device/PWA-Install braucht HTTPS (Master: Tailscale-serve/Cert).
