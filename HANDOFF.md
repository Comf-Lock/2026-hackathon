...
> **agent/agent-3 Stand 2026-06-26 (Profil-Bug):** Interessen/Expertise-Speichern + „+"-Button **gefixt + gepusht** — bereit für Master-PR. Commit `cb7b323` auf rebased origin/master. `ProfileView.vue` von Chip-UI auf **editierbare Zeilen-Liste** umgebaut: „+" legt neue Eingabezeile an, jede Zeile mit „×"; die Rows **sind** die Daten (v-model aufs Array) → kein getippter-aber-nicht-gechippter Wert geht mehr verloren (das war der Bug). `save()` bereinigt (trim/leer/dedupe) und PUTet interests/expertise/home_label/radius_km; Werte überleben Reload. **`profile.py` NICHT geändert** — `PUT /api/profile` persistierte bereits alles korrekt; Bug war rein Frontend. Layout: Edit-Spalte spiegelt jetzt Landing/Dashboard-Shell (1240 + 854 Content-Spalte) → gleiche Breite/linke Kante wie Event-Feed, kein Sprung. Neuer Backend-Test `test_profile.py`. 122 Backend-Tests grün, vite build grün.
