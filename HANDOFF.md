...
> **agent/agent-4 Stand 2026-06-26:** Kalender-Ansicht **fertig + bereit für Master-PR**. Rebased auf master, Commit [2ad1d5d]. Öffentliche Route `/calendar` (kein Auth, logged-out+in), `Liste/Kalender`-Umschalter im Header. `CalendarView.vue` + `calendar/calendarRange.js`: Woche/Monat/Jahr, lädt sichtbaren Zeitraum über `GET /api/events` (date_from/date_to, limit 100, pro Periode paginiert), eigene kompakte Zellen (EventCard NICHT angefasst). `vite build` grün. Offen/optional: SearchMask über dem Kalender.
...
- **2026-06-26** · agent-4 calendar-view · rebased auf master (8449feb); öffentliche `/calendar` (Woche/Monat/Jahr) gebaut: CalendarView.vue + calendar/calendarRange.js, lädt sichtbaren Zeitraum via GET /api/events (date_from/date_to, paginiert), eigene Zellen (EventCard unberührt), Liste/Kalender-Umschalter in App.vue. vite build grün [2ad1d5d]. agent/agent-4 gepusht → Master-PR offen
