...
created: 2026-06-25
last_updated: 2026-06-26-agent-4-pwa-shell
schema_version: "0.4"
status: slice1-deployed · master-orchestration · agent-4-pwa-shell-phase1a
---
...
agent/agent-4: PWA-Shell (Phase 1a, **nur Shell**) **fertig + gepusht** [f26db4e, auf rebased origin/master]. `vite-plugin-pwa` als devDep → `manifest.webmanifest` (Event Radar, `display: standalone`, theme `#b8324f` / bg `#f7f6f3` Mainfranken, `start_url`/`scope` `/`, Icons 192+512 'any' + maskable-512) + Workbox-Service-Worker (precacht App-Shell, 15 entries; `navigateFallback: /index.html` mit `/api`-Denylist; `devOptions.enabled=false`). `index.html`: `theme-color`, `apple-mobile-web-app-*`-Metas, `apple-touch-icon`, `viewport-fit=cover`; SW-Registrierung via `injectRegister: 'auto'` (registerSW.js) → **App-Code unangetastet**. Icons aus dem Radar-Logo generiert (qlmanage SVG→PNG + sips): `frontend/public/icons/{pwa-192,pwa-512,pwa-maskable-512,apple-touch-icon}.png`. `dev-dist/` zu `.gitignore`. **NUR** index.html/vite.config/package(-lock)/.gitignore/public-icons — keine Views/Komponenten. vite build grün (63 modules), Manifest valide JSON, sw.js+workbox+registerSW erzeugt. **App ist installierbar** (Browser bietet „Installieren"). On-Device-Install-Test braucht HTTPS (Master kümmert sich um Cert/Tunnel).
