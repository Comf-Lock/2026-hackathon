...
created: 2026-06-25
last_updated: 2026-06-26-agent-2-p2-backend-hygiene
schema_version: "0.4"
status: done, ready-to-merge · P2-backend-hygiene-done · awaiting-brief
---
...
**AGENT-2 (Branch agent/agent-2) — P2 Backend-Hygiene abgeschlossen & gepusht [53ad016].** Rebased auf origin/master. Zwei isolierte Backend-Tasks: (1) `/api/auth/me` serviert jetzt `UserOut` (Pydantic `response_model`) statt hand-Dict → expliziter OpenAPI-Vertrag, gleiche JSON-Form. (2) Startup-Secrets-Guard in `main.py`-Lifespan (`_guard_secrets`): warnt in Dev / **fail-fast in Production** wenn `session_secret` oder Google-OAuth-Creds noch Dev-Defaults sind; `config.py` mit benannten `DEV_*`-Konstanten + `environment`-Setting + `insecure_defaults()`. Nur `auth.py`/`config.py`/`main.py` berührt. pytest 85 grün. Branch @ 53ad016, Tree clean. **Nächster Schritt:** Auf den nächsten Brief von Master warten (`_scrape/inbox/`). Bei Session-Start ohne neuen Brief: Session parken und melden — keine Arbeit erfinden. Offen nur Master-seitig: P2-PR (+ älterer P1.2-PR) nach master mergen; finaler Browser-Visualcheck.

> Frühere agent-2-Refactors (alle gemergt/gepusht): Visibility-Dedup, P0.4 (DashboardView-Slim), P0.3 (useEvents-Daten-Layer), P1.2 (async→def auth/profile).
...
- **2026-06-26** · agent-2 P2-backend-hygiene · rebased auf origin/master; /api/auth/me → UserOut response_model; Startup-Secrets-Guard (warn dev / fail-fast prod) in main.py + config.py (DEV_*-Konstanten, environment, insecure_defaults). Nur auth/config/main. pytest 85 grün. agent/agent-2 @ 53ad016 gepusht → Master-PR offen
