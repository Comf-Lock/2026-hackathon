...
created: 2026-06-25
last_updated: 2026-06-26-agent-4-views
schema_version: "0.4"
status: slice1-deployed · master-orchestration · agent-4-calendar-map-audit
---
...
agent/agent-4: Drei öffentliche View-Tabs fertig + gepusht (PR-ready auf agent/agent-4). (1) /calendar (Woche/Monat/Jahr) [2ad1d5d]. (2) /map (Leaflet/OSM) [1bbf33b] inkl. zweispaltiger Event-Seitenliste MapEventList.vue mit Klick-zu-Sprung (flyTo+Popup), Events ohne Koordinaten ausgegraut, optionalem Profil-Filter (read-only /api/profile, Toggle) [fe1c441]. Nav Liste/Kalender/Karte im Header. Daten über useEvents (P0.3, vom Master gemergt) bzw. zentrales formatDateLabel in calendarRange; EventCard/useEvents nur konsumiert, nicht umgebaut. vite build grün. Zusätzlich: Architektur-Audit der Code-Basis → priorisierter Refactor-Plan plans/refactor-architecture-audit.md [e0e2478], Info-Notiz liegt in der Master-Inbox. **Nächster Schritt:** Master merged agent/agent-4 via PR(s); Agent-4 wartet auf neuen Brief bzw. Zuteilung agent-4-naher Refactor-Tasks (CalendarView/Map, Doku-Drift, Nav-Bug).
