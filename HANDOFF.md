...
> **agent/agent-4 Stand 2026-06-26:** Zwei öffentliche View-Tabs **fertig + bereit für Master-PR** (gepusht auf `agent/agent-4`). (1) **Kalender** `/calendar` (Woche/Monat/Jahr) [2ad1d5d]. (2) **Karte** `/map` (Leaflet/OSM, Marker mit lat/lng, Popup+Empty-State, `leaflet` als npm-Dep) [1bbf33b]. `Liste/Kalender/Karte`-Umschalter im Header. Beides eigener leichter Fetch via `api()`, EventCard/useEventSearch/DashboardView unangetastet. `vite build` grün. Zusätzlich: **Architektur-Audit + priorisierter Refactor-Plan** (`plans/refactor-architecture-audit.md` [e0e2478]) — Info-Notiz liegt in der Master-Inbox.
...
- **2026-06-26** · agent-4 calendar+map+audit · /calendar (Woche/Monat/Jahr) [2ad1d5d] + /map (Leaflet/OSM) [1bbf33b], Liste/Kalender/Karte-Umschalter, eigener api()-Fetch (fremde Dateien unberührt), vite build grün. Architektur-Audit → priorisierter Refactor-Plan [e0e2478], Info an Master-Inbox. agent/agent-4 gepusht → Master-PR(s) offen
