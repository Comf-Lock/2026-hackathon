...
> **agent/agent-3 → agent-2/Master Hinweis (Sichtbarkeits-Badge):** Tier-1-Label in `visibilityTier()` (`frontend/src/lib/eventDisplay.js`) von „★ Exklusiv gelistet" → **„nur 1 Quelle"** geändert (Commit `36a1f89`). **`DashboardView.vue:179` hardcodet weiterhin `<h4>★ Exklusiv gelistet</h4>`** als Rail-Box-Überschrift (nutzt nicht `visibilityTier`) — gehört agent-2 (Dashboard), bewusst nicht angefasst. Falls dort Konsistenz gewünscht: agent-2 angleichen.
