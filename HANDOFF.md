...
created: 2026-06-25
last_updated: 2026-06-26-agent-2-scraper-research
schema_version: "0.4"
...
**AGENT-2 (Branch agent/agent-2) — Recherche-Brief `task-research-scrapers` abgeschlossen, PR offen (Master merged).** `documentation/features/scraper-sources-research.md` geschrieben [e3c27f7]: HTML-Eventquellen Mainfranken ohne RSS/ICS, die einen Scraper bräuchten. 4 parallele Web-Recherchen, Seiten real abgerufen; 8 neue Kandidaten bewertet (Static-vs-SPA, Felder, Playwright, Wert/Aufwand) + priorisierte Empfehlung. **Kernbefund:** ZDI/AI-Week/THWS/THWS-FIW/Eventbrite/Gründerzentren sind bereits Code-Adapter → in der Doc als redundant markiert (Agent-5 baut sie nicht neu). Genuine Neu-Lücken (Empfehlung, alle statisch, kein Playwright): Informatik-Institut Uni Wü, KI-Regio, Startbahn27 (Schweinfurt), GIM-Aggregator. Reine Recherche, kein Code. Brief nach `_scrape/processed/`. **Hinweis:** `git rebase origin/master` ersetzte HANDOFF.md erneut durch Masters Version (Rebase-Eigenheit — `merge=ours` greift nur bei Merge); agent-2-HANDOFF aus 1129570 wiederhergestellt. **Vorheriger Brief:** enrichment-Refactor → PR#47 (offen). **Nächster Schritt:** Auf den nächsten Brief von Master warten (_scrape/inbox/). Bei Session-Start ohne neuen Brief: Session parken und melden — keine Arbeit erfinden.
