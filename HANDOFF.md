...
> **agent/agent-3 Stand 2026-06-26:** Platzhalter-Gewichtungsbalken **entfernt + gepusht** — bereit für Master-PR. Commit `afb5773` (`feat(card): drop placeholder bar — show only real weights`) auf rebased origin/master. Balken/Themen-Block rendert nur noch für LLM-bewertete Events mit echten `topic_weights`; unbewertete Events zeigen gar keinen Balken (kein Platzhalter, kein Tag-Equal-Split). `PLACEHOLDER_WEIGHTS`/`tagWeights`/`TAG_PALETTE` raus, `weightBar()` → null ohne echte Gewichte. Echte Pills + „geschätzt"-Marker unverändert. Nur `EventCard.vue` + `eventDisplay.js`, vite build grün (61 Module). Brief → `_scrape/processed/`.
