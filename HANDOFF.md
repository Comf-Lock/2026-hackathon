...
> **agent/agent-1 Stand 2026-06-26 (Location-Enrichment):** **fertig + gepusht** — bereit für Master-PR. 2 Commits auf rebased master (49866a0+Dedup). **Teil A** `feat(ingest): extract venue/city/postal/coords` — meetup zieht venue/group lat-lng+city+postal, thws_fiw default city Würzburg (6 Events), eventbrite city/postal aus Street, FRIZZ venue aus '@'-Tail. **Teil B** `feat(enrich): nominatim geocoding` — `geocode.py` Geocoder (cache/≥1req-s/graceful) + `geocode_events(session)` (nur lat IS NULL, idempotent) + CLI `app.ingest geocode` / `run --geocode`. **Kein models.py-Schema-Change** (Cache = Event.lat/lng selbst, kein neues Table → keine Postgres-Migration nötig). 71 pytest grün, Geocoding-Tests mit gemocktem Nominatim (kein echter HTTP-Call). Feed-Kanal davor ist bereits in master. Details: Journal 2026-06-26.
...
- **2026-06-26** · agent-1 location-enrichment · rebased (master hat jetzt Dedup); Teil A Ort-Extraktion in allen Adaptern (meetup coords/city/postal + group-fallback, thws→Würzburg, eventbrite street→city/postal, FRIZZ venue), Teil B Nominatim-Geocoding (geocode_events, cache/1req-s/graceful/idempotent, CLI geocode). Kein Schema-Change. 71 pytest grün (Geocoding gemockt). gepusht → Master-PR
