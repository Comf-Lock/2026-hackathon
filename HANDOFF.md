...
> **agent/agent-1 Stand 2026-06-26 (12:32):** Zwei Tasks fertig + gepusht zu origin/agent/agent-1, bereit für Master-PRs nach master:
> 1. **Radius-Suche (Haversine)** [`bcf522e`] — geteilter `backend/app/geo.haversine_km` (ingest/filters importiert ihn, keine 2. Distanz-Impl); `GET /api/events` optionale `lat/lng/radius_km` (nur zusammen aktiv, Events ohne Koordinaten ausgeschlossen, sonst unverändert); Frontend SearchMask Radius-Slider + 'In meiner Nähe' (geolocation) gated auf `geo`-Prop, useEvents `geo`-Option resolved Zentrum (Profil home eingeloggt, sonst geolocation), LandingView geo:true. **Hinweis:** logged-in Profil-Zentrum greift, sobald DashboardView (Agent-2) `useEvents({ geo: true })` setzt — bewusst nicht editiert (Eigentums-Grenze).
> 2. **Refactor API/Service-Logging (P1.3)** [`bd5a2cc`] — per-Modul-Logger (eventradar.auth/profile/events), benannte Konstanten (DEFAULT_PAGE_SIZE/MAX_PAGE_SIZE in events_service vom Router importiert, LAT/LNG_MIN/MAX, DEFAULT_RADIUS_KM), Verhalten unverändert.
> 3. **tag-Filter als Freitext** [`1e55f22`] — `tag` matcht jetzt als case-insensitive Substring in title/description/organizer/tags (mehrere Terme OR), statt striktem Array-Membership → Pitch-Demo findet Treffer. q/no-tag unverändert.
> 4. **OAuth redirect_uri host-abhängig** [`5497379`] — `auth.py` leitet die Callback-URL aus `X-Forwarded-Host`/`X-Forwarded-Proto` ab (Fallback localhost-Default), damit Login über localhost UND Tailscale-HTTPS (ts.net) funktioniert. **⚠️ Ops/Master:** in der Google Console müssen BEIDE Callback-URLs eingetragen werden: `http://localhost:8000/api/auth/google/callback` UND `https://macbook-pro-von-lars.tail7629bb.ts.net/api/auth/google/callback`.
>
> 134 pytest + vite build grün. **Nächster Schritt:** kein offener agent-1-Code-Task; Master merged die 4 Commits (bcf522e/bd5a2cc/1e55f22/5497379) via PR nach master.
